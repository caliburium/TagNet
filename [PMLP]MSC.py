...
class PartitionedMLP(nn.Module):
    def __init__(self, num_classes=10, n_domains=3, hidden_size=3072):
        ...
        for _ in range(n_domains):
            classifier = nn.Sequential(
                nn.Linear(hidden_size, partition_size),
                nn.ReLU(),
                nn.Linear(partition_size, num_classes)
            )
...
def main():
    ...
    wandb.init(entity="hails",
               ...
               name="[PMLP]MSC_" + str(args.hidden_size)
                    + "_lr:" + str(args.lr)
    ...
    model = PartitionedMLP(num_classes=10, n_domains=3, hidden_size=args.hidden_size).to(device)
    ...
    for epoch in range(num_epochs):
        ...
        def tester(loader, domain_id, group_name):
            label_correct, total_label_loss, total = 0, 0, 0

            for images, labels in loader:
                images, labels = images.to(device), labels.to(device)

                class_output = model(images, domain_id=domain_id)
