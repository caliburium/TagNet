...
class TagNetMLP(nn.Module):
    def __init__(self, num_classes=10, num_partition=3, num_domains=3,
                 fc_hidden=256, disc_hidden=256,
                 device='cuda' if torch.cuda.is_available() else 'cpu'):
        super(TagNetMLP, self).__init__()
        self.device = device
        ...
        for p_i in range(self.num_partition):
            ...
            for i, linear_layer in enumerate(linear_layers):
                if i == 0:
                    ...
                    input_size = linear_layer.in_features
                    output_size = linear_layer.out_features
                    partition_size = input_size // self.num_partition

                    partitioned_layer.append(nn.Linear(partition_size, output_size))
...
class TagNetMoE(nn.Module):
    def __init__(self, num_classes=10, num_partition=3, num_domains=3,
                fc_hidden = 768, disc_hidden = 256,
                device = 'cuda' if torch.cuda.is_available() else 'cpu'):
        super(TagNetMoE, self).__init__()
        self.device = device
        self.to(self.device)
