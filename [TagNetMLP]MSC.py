...
def run_epoch(mode, model, loaders, criterion, optimizer, epoch, args, tau, lambda_p):
    ...
    len_dataloader = max(len(loaders['mnist']), len(loaders['svhn']), len(loaders['cifar']))
...
def main():
    ...
    parser.add_argument('--fc_hidden', type=int, default=3072)
    ...
    wandb_run = wandb.init(entity="hails",
                           ...
                           name="[TagnetMLP]MSC_lr:" + str(args.lr)
