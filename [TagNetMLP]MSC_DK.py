...
def train_step(epoch, model, args, optimizer, criterion, domain_criterion, data_loader_zip, phi, lambda_p, tau,
               inference=False):
    if not inference:
        model.train()
    else:
    ...
    for i, (mnist_data, svhn_data, cifar_data) in enumerate(data_loader_zip):
        ...
        total_label_loss += label_loss.item() * (bs_m + bs_s + bs_c)
...
def main():
    ...
    for epoch in range(num_epochs):
        ...
        phi = math.exp(-epoch / num_epochs * 5)
