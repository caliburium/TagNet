...
def main():
    ...
    parser.add_argument('--switcher_lr', type=float, default=0.5)
    ...
    furniture_real_train, furniture_real_test = dn_loader('real', [301, 302, 291, 328, 340, 282, 316, 294, 164, 221], args.batch_size)
    furniture_quickdraw_train, furniture_quickdraw_test = dn_loader('quickdraw',[301, 302, 291, 328, 340, 282, 316, 294, 164, 221], args.batch_size)

    tool_real_train, tool_real_test = dn_loader('real', [196, 299, 37, 18, 243, 14, 226, 11, 206, 7], args.batch_size)
    tool_painting_train, tool_painting_test = dn_loader('painting', [196, 299, 37, 18, 243, 14, 226, 11, 206, 7], args.batch_size)

    mammal_real_train, mammal_real_test = dn_loader('real', [281, 91, 336, 311, 343, 258, 103, 147, 64, 186], args.batch_size)
    mammal_paint_train, mammal_paint_test = dn_loader('painting', [281, 91, 336, 311, 343, 258, 103, 147, 64, 186], args.batch_size)
    ...
    for epoch in range(num_epochs):
        ...
        with ((torch.no_grad())):
            ...
            test_fr_avg_loss, test_fr_dom_avg_loss, test_fr_acc, test_fr_dom_acc, test_fr_ratios, test_fr_label_counts, fr_gumbel = evaluate_dataset(furniture_real_test, 0)
            test_fq_avg_loss, test_fq_dom_avg_loss, test_fq_acc, test_fq_dom_acc, test_fq_ratios, test_fq_label_counts, fq_gumbel = evaluate_dataset(furniture_quickdraw_test, 0)
            test_tr_avg_loss, test_tr_dom_avg_loss, test_tr_acc, test_tr_dom_acc, test_tr_ratios, test_tr_label_counts, tr_gumbel = evaluate_dataset(tool_real_test, 1)
            test_tp_avg_loss, test_tp_dom_avg_loss, test_tp_acc, test_tp_dom_acc, test_tp_ratios, test_tp_label_counts, tp_gumbel = evaluate_dataset(tool_painting_test, 1)
            test_mr_avg_loss, test_mr_dom_avg_loss, test_mr_acc, test_mr_dom_acc, test_mr_ratios, test_mr_label_counts, mr_gumbel = evaluate_dataset(mammal_real_test, 2)
            test_mp_avg_loss, test_mp_dom_avg_loss, test_mp_acc, test_mp_dom_acc, test_mp_ratios, test_mp_label_counts, mp_gumbel = evaluate_dataset(mammal_paint_test, 2)

            current_avg_acc = (test_fr_acc + test_fq_acc + test_tr_acc + test_tp_acc + test_mr_acc + test_mp_acc) / 6.0
            test_label_avg_loss = (test_fr_avg_loss + test_fq_avg_loss + test_tr_avg_loss + test_tp_avg_loss + test_mr_avg_loss + test_mp_avg_loss) / 6.0
            test_domain_avg_loss = (test_fr_dom_avg_loss + test_fq_dom_avg_loss + test_tr_dom_avg_loss + test_tp_dom_avg_loss + test_mr_dom_avg_loss + test_mp_dom_avg_loss) / 6.0

            furniture_gumbel = torch.cat((fr_gumbel, fq_gumbel), dim=0)
            tool_gumbel = torch.cat((tr_gumbel, tp_gumbel), dim=0)
            mammal_gumbel = torch.cat((mr_gumbel, mp_gumbel), dim=0)
            ...
            loss_spec_t = -torch.sum(avg_prob_tool * torch.log(avg_prob_tool + 1e-8))
            loss_spec_m = -torch.sum(avg_prob_mammal * torch.log(avg_prob_mammal + 1e-8))
            test_specialization_loss = (loss_spec_f + loss_spec_t + loss_spec_m).item()
            ...
            test_fr_log = get_label_partition_log_data(test_fr_label_counts, 'F-R', args.num_classes,args.num_partition, "Test")
            test_fq_log = get_label_partition_log_data(test_fq_label_counts, 'F-Q', args.num_classes,args.num_partition, "Test")
            test_tr_log = get_label_partition_log_data(test_tr_label_counts, 'T-R', args.num_classes,args.num_partition, "Test")
            test_tp_log = get_label_partition_log_data(test_tp_label_counts, 'T-P', args.num_classes,args.num_partition, "Test")
            test_mr_log = get_label_partition_log_data(test_mr_label_counts, 'M-R', args.num_classes,args.num_partition, "Test")
            test_mp_log = get_label_partition_log_data(test_mp_label_counts, 'M-P', args.num_classes,args.num_partition, "Test")
